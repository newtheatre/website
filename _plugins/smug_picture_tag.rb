# Title: Smug Picture Tag
# Author: Will Pimblett
#
# Adaptation of Jekyll Picture Tag to use SmugMug as image storage backend

# Original: Jekyll Picture Tag by Rob Wierzbowski : @robwierzbowski
...
module Jekyll
  ...
  class Picture < Liquid::Tag
    ...
    def render(context)
      ...
      settings = site.config['smug_picture']
      url = site.config['url']
    ...
    def generate_image(instance, site_source, site_dest, image_source, image_dest, baseurl)
      image_url = instance[:src]
      ext = File.extname(instance[:src]) # TODO: determine extension
      basename = File.basename(instance[:src], ext) # TODO: determine basename

      # TODO: download image if we don't already have
